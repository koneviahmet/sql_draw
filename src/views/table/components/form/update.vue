  ...
  <div class="m-4">
    <form class="w-full max-w-sm">
      ...
      <div class="form-control mt-6">
        <a @click="update" class="btn btn-primary" :class="tableLoading && 'loading btn-disabled'">update</a>
      </div>
    </form>   
  </div>   
</template>
...
export default {
    setup(){ 
        ...
        const {tableLoading, tableError, updateTable, getTable} = useTable();
        ...
        const update = () => {
            validate().then(validateSuccess => {
                ...
                if(validateSuccess.valid){
                    updateTable({id:route.params.id,...values}).then(response => {
