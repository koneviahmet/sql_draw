<template>
  ...
  <Skeletor :loading="tableLoading" :length="4"/>
      ...
      <tbody>
        <tr v-for="item in tableData" :key="item.id">
          ...
          <td class="flex space-x-1">
            <router-link :to="`/table/detail/${item.id}`" class="btn btn-sm">Select</router-link>
...
const { tableLoading, tableData, tableError, getTables, deleteTable } = useTable();
onMounted(() => getTables());
