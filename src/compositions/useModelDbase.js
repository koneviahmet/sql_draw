...
import DbaseServices from "../services/DbaseServices";
...
export default function () {
  ...
  watch(data, () => {
    store.commit("addDbase", [...data.list])
    //console.log("changed",store.getters._getDbase);
  })

  const getItems = async () => {
    loading.value = true;
    return new Promise(async (resolve, reject) => {
      await DbaseServices.getItems()
        .then((response) => {
          loading.value = false;
  ...
  const searchItems = async (obj) => {
    loading.value = true;
    return new Promise(async (resolve, reject) => {
      await DbaseServices.searchItems(obj)
        .then((response) => {
          loading.value = false;
          if (response && !response?.error) {
            data.list = [...response];
  ...
  const getItem = async (obj) => {
    loading.value = true;
    return new Promise(async (resolve, reject) => {
      await DbaseServices.getItem(obj)
        .then((response) => {
          loading.value = false;
          if (response && !response?.error) {
            //data.list = {...response};
  ...
  const addItem = async (obj) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      await DbaseServices.addItem(obj)
        .then((response) => {
          loading.value = false;
          if (response && !response?.error) {
  ...
  const updateItem = async (obj) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      await DbaseServices.updateItem(obj)
        .then((response) => {
          loading.value = false;
  ...
  const deleteItem = async (obj) => {
    ...
    loading.value = true;
    return new Promise(async (resolve, reject) => {
      await DbaseServices.deleteItem(obj)
        .then((response) => {
          loading.value = false;
  ...
  const getStoreData = computed(() => {
    return store.getters._getDbase;
  })

  return {
    dbaseError: error,
    dbaseLoading: loading,
    //dbaseData: toRefs(data).list,
    dbaseData: getStoreData,
    getDbase: getItem,
    searchDbases: searchItems,
    getDbases: getItems,
    addDbase: addItem,
    updateDbase: updateItem,
    deleteDbase: confirmDelete,
  };
}
