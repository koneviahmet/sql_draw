...
export default function () {
  ...
  watch(data, () => {
    store.commit("addCreator", [...data.list])
    //console.log("changed",store.getters._getCreator);
  })

  const getItems = async () => {
    loading.value = true;
    return new Promise(async (resolve, reject) => {
      await CreatorServices.getItems()
        .then((response) => {
          loading.value = false;
  ...
  const searchItems = async (obj) => {
    loading.value = true;
    return new Promise(async (resolve, reject) => {
      await CreatorServices.searchItems(obj)
        .then((response) => {
          loading.value = false;
          if (response && !response?.error) {
            data.list = [...response];
  ...
  const getItem = async (obj) => {
    loading.value = true;
    return new Promise(async (resolve, reject) => {
      await CreatorServices.getItem(obj)
        .then((response) => {
          loading.value = false;
          if (response && !response?.error) {
            //data.list = {...response};
  ...
  const addItem = async (obj) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      await CreatorServices.addItem(obj)
        .then((response) => {
          loading.value = false;
          if (response && !response?.error) {
  ...
  const updateItem = async (obj) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      await CreatorServices.updateItem(obj)
        .then((response) => {
          loading.value = false;
  ...
  const deleteItem = async (obj) => {
    ...
    loading.value = true;
    return new Promise(async (resolve, reject) => {
      await CreatorServices.deleteItem(obj)
        .then((response) => {
          loading.value = false;
  ...
  const getStoreData = computed(() => {
    return store.getters._getCreator;
  })

  return {
    creatorError: error,
    creatorLoading: loading,
    //creatorData: toRefs(data).list,
    creatorData: getStoreData,
    getCreator: getItem,
    searchCreators: searchItems,
    getCreators: getItems,
    addCreator: addItem,
    updateCreator: updateItem,
    deleteCreator: confirmDelete,
  };
}
